"""
newsletter.py
Blueprint Flask pour la newsletter gratuite - liste Brevo #4
"""
...
def add_to_brevo_newsletter(email):
    ...
    print(f"Newsletter add: {r.status_code} {r.text}")
...
def remove_from_brevo_newsletter(email):
    ...
    print(f"Newsletter remove: {r.status_code}")
    return r.status_code in [200, 201, 204]
      ...
      <table width="600" cellpadding="0" cellspacing="0"
        ...
        <tr>
          <td style="padding:32px;">
            <h1 style="color:#1a1f16;font-size:1.3rem;margin:0 0 16px;">Inscription confirmée</h1>
        ...
        <tr>
          <td style="background:#f5f8f3;padding:16px 32px;
            ...
            <p style="margin:0;font-size:0.72rem;color:#aaa;">
              ...
              <a href="https://xmath-carte-production.up.railway.app/desabonnement-newsletter?email={email}"
...
@newsletter_bp.route('/inscription-newsletter', methods=['POST'])
...
@newsletter_bp.route('/desabonnement-newsletter', methods=['GET'])
def desabonnement_newsletter():
    email = request.args.get('email', '').strip()
    if not email:
        return redirect('https://emplois-scolaires-monde.online?status=erreur')
    remove_from_brevo_newsletter(email)
    return redirect('https://emplois-scolaires-monde.online?status=desabonne')
